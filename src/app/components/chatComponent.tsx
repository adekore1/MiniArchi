    ...
    <div className=" h-screen flex flex-col bg-neutral-100 dark:bg-neutral-900 text-neutral-800 dark:text-neutral-100 px-4 py-6">
      ...
      <div className=" shadow-xl flex-1 overflow-y-auto space-y-3 rounded-xl p-4 bg-gray-100 dark:bg-neutral-800/60 border dark:border-neutral-700 backdrop-blur-sm">
        {messages.map((m, i) => (
            <div
              key={i}
              className={clsx(
                "shadow-xl max-w-[75%] p-3 rounded-2xl text-sm whitespace-pre-wrap transition-transform duration-300 ease-in-out",
                m.role === "user"
      ...
      >
        ...
        {/* <button
          ...
          disabled={!input.trim()}
          className="flex-1 px-4 py-2 min-h-[48px] rounded-md bg-green-600 hover:bg-green-700 text-white font-medium transition-colors"
        >
          🎨 Generate
        </button> */}
      </form>
